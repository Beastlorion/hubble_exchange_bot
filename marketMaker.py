...
async def orderUpdater(
    client: HubbleClient, hedgeClient: HyperLiquid | Binance | None, marketID, settings, active_order_direction
):
    ...
    while True:
        ...
        availableMargin = (float(positions.margin) - float(positions.reservedMargin if positions.reservedMargin else 0)) * float(settings["marginShare"])
            ...
            try:
                ...
                for idx, order in enumerate(placed_orders):
                    ...
                    if order["success"] == True:
                        ...
                        active_order_direction[order['order_id']] = 1 if quantity > 0 else -1
                    else:
                        print(
